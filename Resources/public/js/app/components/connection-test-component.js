...
class ConnectionTestComponent extends BaseComponent {
    ...
    onTest() {
        ...
        $.ajax({ url, method: 'GET', data: { overrides: this.collectOverrides() } })
            .done((response) => this.render(response))
            .fail((jqXhr) => {
    ...
    collectOverrides() {
        ...
        $scope.find('[name^="' + prefix + '"][name$="' + suffix + '"]').each((_index, el) => {
    ...
    render(response) {
        ...
        this.$result.append($('<div/>', { 'class': 'aaxis-conn-test__message' }).append($('<span/>', { 'class': 'fa ' + icon + ' aaxis-conn-test__icon', 'aria-hidden': 'true' }), $('<span/>', { text: response.message || '' })));
        ...
        if (keys.length > 0) {
            ...
            keys.forEach(key => {
                ...
                $list.append($('<li/>').append($('<span/>', { 'class': 'aaxis-conn-test__detail-key', text: key + ': ' }), $('<span/>', { text: String(details[key]) })));
            });
